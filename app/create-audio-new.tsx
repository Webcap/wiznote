...
export default function CreateAudioNoteScreen() {
  ...
  const startRecording = async () => {
    try {
      console.log('[CreateAudio] Starting recording...');
      ...
      console.error('[CreateAudio] Error starting recording:', error);
  ...
  const stopRecording = async () => {
    try {
      console.log('[CreateAudio] Stopping recording...');
      
      if (!isRecording) return;

      // Get the current recording from AudioUtils
      // Note: This is a simplified approach - in a real app you'd store the recording reference
      setIsRecording(false);
      setHasRecording(true);
      setProgress(25);
      setProgressMessage('Processing recording...');

      // For now, we'll simulate the recording completion
      // In a real implementation, you'd get the actual recording URI
      const mockUri = `file:///tmp/recording_${Date.now()}.m4a`;
      setAudioUri(mockUri);
      setProgress(50);
      ...
      console.error('[CreateAudio] Error stopping recording:', error);
                ...
                <View style={styles.recorderControls}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.recordButton,
                      { 
                        backgroundColor: isRecording ? '#FF6B6B' : accentPrimary,
                      },
                    ]}
                    ...
                    <View style={styles.durationContainer}>
                      {isRecording && (
                        <View style={styles.recordingIndicator}>
                          <View style={[styles.recordingDot, { backgroundColor: accentPrimary }]} />
                        </View>
                      )}
